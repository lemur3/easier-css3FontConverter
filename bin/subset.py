...
def getsubset(subset, font_in):
    ...
    quotes += [0x2026] # ellipses
    quotes += [0x2039] # guilsinglleft
    quotes += [0x203A] # guilsinglright
    quotes += [0x2044] # fraction slash

    latin  = range(0x20, 0x7f) # Basic Latin (A-Z, a-z, numbers)
    ## latin += range(0xa0, 0x100) # Western European symbols and diacritics
    latin += [0x20ac] # Euro
    latin += [0x00a3] # gbp pound
    latin += [0x00a9] # copyright
    latin += [0x00ab] # double left arrow
    latin += [0x00ae] # registered mark
    latin += [0x00b0] # degrees mark
    latin += [0x00b4] # upper e accent mark cafe
    latin += [0x00b7] # latin bullet mark
    latin += [0x00bb] # double right arrow
    latin += [0x00b1] # plus or minus
    latin += [0x00b2] # super 2
    latin += [0x00b3] # super 3
    latin += [0x00b6] # paragraph mark
    latin += [0x00b9] # super 1
    latin += [0x00ba] # super 0
    latin += [0x00bc] # onequarter
    latin += [0x00bd] # onehalf
    latin += [0x00be] # three quarter
    latin += [0x00d7] # multiplication
    latin += [0x00f7] # division
    latin += [0x0152] # OE
    latin += [0x0153] # oe
    ...
    latin += [0x02c2] # less than
    latin += [0x02c3] # greater than
    latin += [0x02da] # ring
    latin += [0x02dc] # tilde
    latin += [0x007e] # latin tilde
    latin += [0x2074] # foursuperior
    latin += [0x2075] # fivesuperior
    latin += [0x2076] # sixsuperior
    latin += [0x2077] # sevensuperior
    latin += [0x2078] # eightsuperior
    latin += [0x2079] # ninesuperior    
    latin += [0x2116] # numero sign
    latin += [0x2122] # trademark
    latin += [0x2215] # divison slash
    latin += [0x2044] # fraction slash
    latin += [0x221e] # infinity
    latin += [0x2153] # one third
    latin += [0x2154] # two third
    latin += [0x215b] # one eighth
    latin += [0x215c] # three eight
    latin += [0x215d] # five eighth
    latin += [0x215e] # seven eighth
    ...
    latin += range(0xFB00, 0xFB13) # alphabetix presentation forms ligatures
    latin += range(0xfe20, 0xfe24) # combing half marks ligatures
    result = quotes
